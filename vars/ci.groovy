      ...
      if(env.JOB_BASE_NAME ==~ "PR.*") {
          ...
          stage('Code Quality') {
              ...
              wrap([$class: 'MaskPasswordsBuildWrapper', varPasswordPairs: [[password: "${SONAR_TOKEN}", var: 'PASSWORD']]]) {
                  sh '/tmp/sonar-scanner-5.0.0.2966-linux/bin/sonar-scanner -Dsonar.host.url=http://172.31.10.101:9000/ -Dsonar.login=${SONAR_TOKEN} -Dsonar.projectKey=expense-backend -Dsonar.projectName=expense-backend -Dsonar.exclusions=node_modules/**'
              }
          }
